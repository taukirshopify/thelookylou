...
class QuantityInput extends HTMLElement {
  ...
  onButtonClick(event) {
    ...
    //subtotalElement.html(Shopify.formatMoney(subtotal+'.00', DT_THEME.moneyFormat));
...
if ($('.main-product-template').length > 0) {
class Accordion {
  constructor(el) {
    this.el = el;
...
class GridSlider extends HTMLElement {
  ...
  init() {
    ...
    const container = this.slider.querySelector('[data-grid-slider]');
    const initiate = new Swiper(container, option);
    initiate.on('init', () => {
    initiate.update();
    });
...
// customElements.define('swiper-testimonials', Testimonials);